...
async function compile({
  ...
  if (fields.some(field => field.value)) {
    ...
    const generatedEntryPath = await getGeneratedEntryPath();
    report.warn(`No ${_kleur.default.blue(fields.map(field => field.name).join(' or '))} field found in ${_kleur.default.blue('package.json')}. Consider ${generatedEntryPath ? `pointing it to ${_kleur.default.blue(generatedEntryPath)}` : 'adding it'} so that consumers of your package can use it.`);
  }
}
