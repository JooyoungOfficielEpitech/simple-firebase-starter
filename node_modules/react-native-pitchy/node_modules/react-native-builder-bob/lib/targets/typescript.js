...
async function build({
  ...
  try {
    ...
    const result = _crossSpawn.default.sync(tsc, ['--pretty', '--declaration', '--declarationMap', '--noEmit', 'false', '--emitDeclarationOnly', '--project', project, '--outDir', output], {
      stdio: 'inherit',
      cwd: root
    });
    if (result.status === 0) {
      ...
      if (fields.some(field => field.value)) {
        await Promise.all(fields.map(async ({
          ...
          const typesPath = _path.default.join(root, value);
