...
const forgotPasswordSchema = z.object({
  email: z
    .string()
    .email({ message: translate("auth:validation.emailInvalid") }),
});
...
export const ForgotPasswordScreen: FC<ForgotPasswordScreenProps> =
  function ForgotPasswordScreen({ navigation }) {
    ...
    const validateEmailForPasswordReset = useCallback(
    ...
    const handleSendResetEmail = useCallback(async () => {
      try {
        ...
        const canResetPassword = await validateEmailForPasswordReset(
          data.email,
        );
        ...
        Alert.alert(
          ...
          error instanceof Error
            ? error.message
            : translate("forgotPasswordScreen:sendFailed"),
        );
      } finally {
        setIsLoading(false);
      }
      ...
      >
        <View style={themed($headerContainer)}>
          ...
          <Text
            tx={
              isEmailSent
                ? "forgotPasswordScreen:subtitleSent"
        ...
        <View style={themed($formContainer)}>
          {!isEmailSent ? (
            ...
            <View style={themed($sentContainer)}>
              <Text
                tx="forgotPasswordScreen:checkEmail"
